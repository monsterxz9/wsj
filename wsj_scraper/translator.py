...
class AITranslator:
    ...
    def _parse_json_fallback(self, text: str) -> dict:
        """备用 JSON 解析：处理尾随逗号、markdown 代码块包裹等常见问题"""
        # 去除 markdown 代码块
        text = re.sub(r"^```json\s*", "", text.strip())
        text = re.sub(r"^```\s*", "", text)
        text = re.sub(r"\s*```$", "", text)

        # 先修复尾随逗号再解析（Gemini 最常见的输出问题）
        ...
        raise ValueError(f"Failed to parse Gemini JSON response: {text[:200]!r}")
