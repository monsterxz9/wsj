...
class WSJScraper:
    ...
    async def _wait_for_homepage_ready(self, page):
        """等待首页链接加载，优先使用动态等待，失败再回退固定等待"""
        selectors = ["main a[href]", "article a[href]", 'a[href*="/world/"]']
    ...
    async def _wait_for_article_ready(self, page):
        """等待文章正文加载，优先使用动态等待，失败再回退固定等待"""
        selectors = ["article p", '[data-type="paragraph"]', ".article-content p"]
        ready_selector = None
        for selector in selectors:
            try:
        ...
        for _ in range(max_rounds):
            try:
                current_count = await page.locator(ready_selector).count()
            except Exception:
                current_count = 0

            if current_count == last_count and current_count >= min_paragraphs:
                stable_rounds += 1
            else:
                stable_rounds = 0
        ...
        # 低段落数场景再给一次缓冲，兼容脚本动态填充慢的页面
        if last_count < min_paragraphs:
            await asyncio.sleep(3)

    async def get_homepage_articles(
        self, limit: int = MAX_ARTICLES_PER_RUN
    ) -> list[str]:
        """从 WSJ 首页获取文章链接"""
        if not self.browser:
        ...
        try:
            ...
            # 等待页面进一步加载（动态等待）
            await self._wait_for_homepage_ready(page)
                ...
                try:
                    ...
                    if "wsj.com/" in href or href.startswith("/"):
                        ...
                        if re.search(r"/[a-z-]+-[a-f0-9]{8}(\?|$)", href):
                            ...
                            if clean_url not in seen_urls and not self.is_processed(
                                clean_url
                            ):
    ...
    async def scrape_article(self, url: str) -> Optional[Article]:
        ...
        try:
            ...
            # 等待内容加载（动态等待正文；必要时回退固定等待）
            await self._wait_for_article_ready(page)
            ...
            for sel in ["article p", ".article-content p", '[data-type="paragraph"]']:
                ...
                for elem in elems:
                    ...
                    if (
                        text
                        and len(text) > MIN_ARTICLE_LENGTH
                        and not self._is_noise(text)
                    ):
